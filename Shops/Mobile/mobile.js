...
const loadDataLowToHigh = async () => {
    ...
    // console.log(pric);
    try {
        const res = await fetch('http://localhost:3000/mobile?_sort=price&_order=asc');
        data = await res.json();
...
const loadDataHighToLow = async () => {
    try {
        const res = await fetch('http://localhost:3000/mobile?_sort=price&_order=desc');
        data = await res.json();
        displayCharacters(data);
    } catch (err) {
        console.error(err);
...
const loadData = async () => {
    try {
        const res = await fetch('http://localhost:3000/mobile');
        data = await res.json();
        displayCharacters(data);
    } catch (err) {
        console.error(err);
