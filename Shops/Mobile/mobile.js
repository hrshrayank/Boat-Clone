...
const loadDataLowToHigh = async () => {
  ...
  try {
    const res = await fetch(
      'https://boatdatabase.herokuapp.com/mobile?_sort=price&_order=asc'
    )
    data = await res.json()
...
const loadDataHighToLow = async () => {
  try {
    const res = await fetch(
      'https://boatdatabase.herokuapp.com/mobile?_sort=price&_order=desc'
    )
    data = await res.json()
    displayCharacters(data)
  } catch (err) {
...
const loadData = async () => {
  try {
    const res = await fetch('https://boatdatabase.herokuapp.com/mobile')
    data = await res.json()
    displayCharacters(data)
  } catch (err) {
    console.error(err)
