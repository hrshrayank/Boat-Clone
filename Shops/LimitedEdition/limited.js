...
const loadDataLowToHigh = async () => {
    try {
        const res = await fetch('http://localhost:3000/homeLimited_Edition?_sort=price&_order=asc');
        data = await res.json();
...
const loadDataHighToLow = async () => {
    try {
        const res = await fetch('http://localhost:3000/Limited_Edition?_sort=price&_order=desc');
        data = await res.json();
        displayCharacters(data);
    } catch (err) {
        console.error(err);
...
const loadData = async () => {
    try {
        const res = await fetch('http://localhost:3000/Limited_Edition');
        data = await res.json();
        displayCharacters(data);
    } catch (err) {
        console.error(err);
